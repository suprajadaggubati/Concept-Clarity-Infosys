...
---
💡 **Note**: This explanation is generated based on general knowledge. For more detailed information, try searching online resources.`
      }
    ...
    // Step 5: Return formatted response with heading and source
...
---
💡 **Note**: This is a general explanation. For more details, try rephrasing your question.`
      }
    } catch (fallbackErr) {
 ...
 * Ensures consistent structure: Meaning, Description, Example, Where it is used
 */
function formatWikipediaFallback(wikipediaData) {
  const { title, content, url } = wikipediaData
  ...
  // Use remaining paragraphs for description
...
**Example**: For specific examples and practical applications of ${title}, please refer to the Wikipedia article linked below. Examples vary depending on the context and field of application.

**Where it is used**: ${title} is used in various contexts and applications. The specific uses depend on the field and domain. See the source link below for detailed information about applications and use cases.
